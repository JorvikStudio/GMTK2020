...
export class Level1Scene extends Scene {
    ...
    create() {
        ...
        this.spellList = ['fireball', 'firecircle', 'smite'];
        ...
        this.enemies.add(new Enemy5(this, 1400, 400, 1150, 1600));
        ...
        this.physics.add.collider(this.spells, this.mainLayer, (spell) => {
            if (spell.onImpact) {
                spell.onImpact();
            }
        });
    ...
    update() {
        ...
        this.physics.overlap(this.player, this.enemies, (player) => {
    ...
    dealPlayerDamage(player, damage) {
        this.load.image("sky", background); player.damage(damage);
        this.load.tilemapTiledJSON("map", gino_level);
    }
}